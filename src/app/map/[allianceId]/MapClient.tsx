...
import type { Alliance, MapConfig, Obj, Meta } from '../../types';
...
export default function MapViewerPage() {
  ...
  const handleCopyMap = async (targetMapId: string) => {
    ...
    try {
      ...
      const res = await fetch(`${base}?action=copyMap&allianceId=${alliance.allianceId}`, {
        method: "POST",
        body: JSON.stringify({
  ...
  const toggleMapVisibility = async (mapId: string) => {
    ...
    try {
      ...
      const res = await fetch(`${base}?action=updateMapConfig&allianceId=${alliance.allianceId}`, {
        method: "POST",
        body: JSON.stringify({
          mapId: mapId,
  ...
  const moveMap = async (mapId: string, direction: 'up' | 'down') => {
    ...
    try {
      ...
      const res = await fetch(`${base}?action=updateAllMapConfigs&allianceId=${alliance.allianceId}`, {
        method: "POST",
        body: JSON.stringify({
          configs: updates,
        }),
